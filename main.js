...
client.once('ready', () => {
    ...
    commands?.create({
        name: 'feedback',
        description: '・Give your feedback on the service'
    })
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isCommand()) {
        ...
        if (customId == 'ticketmenu') {
            ...
            for (let i = 0; i < config.categorys.length; i++) {
                if (values[0] === config.categorys[i].name.toLowerCase().replace(/ /g, "-")) {
                    ...
                    if (channels.size < config.settings.maxticketsopen) {
                        ...
                        await interaction.deferReply({ephemeral: true});
                        await interaction.editReply({content: "You have the maximum number of tickets open", ephemeral: true});
                    }
                }
            }
            ...
            await interaction.deferReply({ephemeral: true});
            const channel = interaction.channel;
            await interaction.editReply({content: "Ticket will be closed", ephemeral: true});
